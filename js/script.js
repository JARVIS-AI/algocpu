...
function addremove() { //add remove bt-io time pair add event listener
    ...
    for (let i = 0; i < process; i++) {
        addbtns[i].onclick = () => {
            ...
            newcell2.innerHTML = '<input class="form-control form-control-sm" type="number" min="1" step="1" value="1">';
            ...
            newcell4.innerHTML = '<input class="form-control form-control-sm" type="number" min="1" step="1" value="1">';
...
function addProcess() {
    ...
    let rowHTML1 =
        `
        <td class="process-id" rowspan="2"><p style="margin-top: 1rem;">P${process}</p></td>
        <td class="priority hide" rowspan="2"><input class="form-control form-control-sm" type="number" min="1" step="1" value="1" style="margin-top: 1rem;"></td>
        <td class="arrival-time" rowspan="2"><input class="form-control form-control-sm" type="number" min="0" step="1" value="0" style="margin-top: 1rem;"> </td>
        <td class="process-time cpu process-heading" colspan="">CPU</td>
        <td class=""><button type="button" class="add-process-btn btn btn-outline-dark fa fa-plus"></button></td>
        <td class=""><button type="button" class="remove-process-btn btn btn-outline-dark fa fa-minus"></button></td>
        `;
    let rowHTML2 =
        `
        <td class="process-time cpu process-input"><input class="form-control form-control-sm" type="number" min="1" step="1" value="1"> </td>
        `;
...
class Output {
    constructor() {
        this.completionTime = [];
        this.turnAroundTime = []; // Turn Around Time = Completion Time – Arrival Time
        this.waitingTime = []; // Waiting time = Turn Around Time – Burst Time
        this.startTime = []; // Start Time = Completion Time - Burst Time
        this.responseTime = [];
        this.schedule = [];
        this.timeLog = [];
...
function setOutput(input, output) {
    //set turn around time and waiting time and start time
    for (let i = 0; i < process; i++) {
        ...
        output.startTime[i] = output.completionTime[i] - input.totalBurstTime[i];
    }
...
function showFinalTable(input, output, outputDiv) {
    ...
    table.classList.add("final-table","table", "table-striped", "table-bordered", "table-hover", "rounded", "rounded-lg", "border", "shadow-lg");
    ...
    for (let i = 0; i < process; i++) {
        ...
        cell.innerHTML= output.startTime[i];
    ...
    const outputArray = outputAverageTimes(output);
    const avgct = outputArray[0];
    const avgtat = outputArray[1];
    const avgwt = outputArray[2];
    const avgrt = outputArray[3];

    let avgct_show = document.createElement("p");
    avgct_show.innerHTML = "Average Completion Time : <span style='font-weight:bold'>" + avgct +"ms</span>";
    avgct_show.classList.add("tooltip-test")
    avgct_show.title.concat("TIIII");
    outputDiv.appendChild(avgct_show);

    let avgtat_show = document.createElement("p");
    avgtat_show.innerHTML = "Average Turn Around Time : <span style='font-weight:bold'>" + avgtat+"ms</span>";
    outputDiv.appendChild(avgtat_show);

    let avgwt_show = document.createElement("p");
    avgwt_show.innerHTML = "Average Waiting Time : <span style='font-weight:bold'>" + avgwt+"ms</span>";
    outputDiv.appendChild(avgwt_show);

    let avgrt_show = document.createElement("p");
    avgrt_show.innerHTML = "Average Response Time : <span style='font-weight:bold'>" + avgrt+"ms</span>";
    outputDiv.appendChild(avgrt_show);
...
//     timeLogDiv.classList.add("container");
//     timeLogDiv.style.height = (15 * process) + 300 + "px";
//     let startTimeLogButtonCon = document.createElement("div");
//     startTimeLogButtonCon.classList.add("container", "text-center");
//     timeLogDiv.appendChild(startTimeLogButtonCon);
//     let startTimeLogButtonRow = document.createElement("div");
//     startTimeLogButtonRow.classList.add("row", "justify-content-md-center");
//     startTimeLogButtonCon.appendChild(startTimeLogButtonRow);
//     let startTimeLogButton = document.createElement("button");
//     startTimeLogButton.id = "start-time-log";
//     startTimeLogButton.classList = "btn btn-primary shadow-lg zoom"
//     startTimeLogButton.innerHTML = "Start Time Log";
//     startTimeLogButton.style.textAlign = "center";
//     startTimeLogButton.style.width = "auto";
//     startTimeLogButtonRow.appendChild(startTimeLogButton);
...
//         timeLogOutputDiv.classList.add("container");
//
//         let timeLogOutputDivRow = document.createElement("div");
//         timeLogOutputDivRow.classList.add("row", "justify-content-md-center");
//
...
//
//         timeLogOutputDiv.appendChild(timeLogOutputDivRow);
//         timeLogOutputDivRow.appendChild(timeLogTableDiv);
//         timeLogOutputDivRow.appendChild(timeLogTime);
...
//     roundRobinChartDiv.classList.add('container');
...
//     algorithmChartDiv.classList.add('container');
...
document.getElementById("calculate").onclick = () => {
    let outputDiv = document.getElementById("output");
    outputDiv.style.marginBottom = '10rem';
    calculateOutput();
};
